...
啟動 TUI 互動介面，填寫專案資訊後自動生成完整的 OpenSpec repo 骨架。

### 情境 2：既有專案導入（Brownfield）

已有專案想套用 OpenSpec 框架，不動既有程式碼：
...
自動掃描當前目錄，注入 `openspec/`、`AGENTS.md`、`.openspec-map.md` 等框架檔案。

### 情境 3：Fork 下來的專案

從 GitHub fork 或 clone 一個專案後，導入 OpenSpec 開發流程：
...
# Step 2: 導入 OpenSpec 框架
/path/to/openspec-toolkit/setup.sh --brownfield

# Step 3: 開始第一個 Change（選用）
/path/to/openspec-toolkit/setup.sh --add-change "add-my-feature"
```
...
在已導入 OpenSpec 的專案中，開始一個新的變更：

```bash
cd /path/to/your-openspec-project
/path/to/openspec-toolkit/setup.sh --add-change "fix-auth-session-leak"
```

### 情境 5：CI/CD 自動化

```bash
/path/to/openspec-toolkit/setup.sh --non-interactive \
...
```bash
export OPENSPEC_HOME="/path/to/openspec-toolkit"
alias openspec="$OPENSPEC_HOME/setup.sh"
```
...
```bash
openspec                                        # 新專案
openspec --brownfield                           # 導入既有專案
openspec --add-change "add-billing-export"      # 追加 Change
```
...
| `./setup.sh --brownfield` | 在當前目錄導入 OpenSpec 框架 |
...
│   ├── openspec-generator/SKILL.md
│   ├── openspec-preflight/
│   │   ├── SKILL.md
│   │   └── openspec-preflight.js
│   └── openspec-brownfield-onboard/SKILL.md
└── templates/
