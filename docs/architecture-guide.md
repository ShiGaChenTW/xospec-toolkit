...
| 情境 | 使用的 Skill |
|------|-------------|
| 建立全新專案（Greenfield） | `xospec-create` — 對話式收集參數，自動生成完整 repo 骨架 |
| 導入既有專案（Brownfield） | `xospec-brownfield-onboard` — 掃描現有結構，最小侵入導入框架 |
| 日常開發（新增 Change / 擴展 Capability） | `xospec-generator` — 動態維護五層結構 |
...
| `spec_delta.md` | Spec Delta — 行為差異 | 記錄這次變更對 living spec 的影響 |
