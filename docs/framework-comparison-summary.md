# Superpowers vs Spec Kit vs x.ospec 研究總結

> 原始資料來源：Perplexity AI 對話（8 輪問答）
> 原始檔案：`Superpowers vs spec-kit vs xospec,可以混用嗎.md`
> 總結日期：2026-04-02
...
| 框架 | 定位 | 核心能力 |
|------|------|----------|
| **x.ospec** | 輕量 spec-driven 規格層 | specs 活在 repo 裡，按 capability 組織，brownfield-first，tool-agnostic |
...
### 推薦組合：x.ospec + Superpowers

- x.ospec 管需求、變更與 spec delta
- Superpowers 管 TDD、review、refactor 等工程紀律
- 兩者不衝突，x.ospec 管「做什麼」，Superpowers 管「怎麼做得好」
...
- x.ospec 把自己定位為 universal planning layer，不綁定特定 AI agent
...
- Kiro 有原生 Specs（Requirements → Design → Tasks），可取代 x.ospec 或 Spec Kit
- 如果用 Kiro 原生 Specs，就不要再疊 x.ospec 或 Spec Kit，否則會雙重管理
- Superpowers 在 Kiro 上可行但屬社群做法，非官方內建
- 結論：如果走 repo-first 路線，Kiro 只當 IDE 宿主，不啟用 Kiro Specs

## 4. x.ospec 兩層架構

對話中深入探討了 x.ospec 的核心設計——把 repo 拆成兩種不同壽命的知識：
...
### 單次變更層：`xospec/changes/<change-id>/`
...
對話中最終確認的架構（後來成為 x.ospec Toolkit 的基礎）：
...
1. 確認 `xospec/specs/<capability>/spec.md` 存在，沒有就先補基本規格
...
```
Read docs/engineering-principles.md and the relevant files under xospec/specs/.
I want to add a new feature.
First, create a new change folder under xospec/changes/<change-id>/.
...
| 決策 | 選擇 | 排除 | 理由 |
|------|------|------|------|
| 主規格系統 | x.ospec | Spec Kit | 更輕量、明確主打 repo-first 和 brownfield-first |
| Workflow 類型 | Repo 檔案驅動 | IDE 一條龍 | 跨工具可攜，不綁定 Kiro/Cursor/任何 IDE |
| Superpowers 角色 | 工程紀律層 | 規格管理 | 與 x.ospec 互補，不重疊 |
| Kiro 角色 | 純 IDE 宿主 | 啟用 Kiro Specs | 避免與 x.ospec 雙重管理 |
| Spec Kit 命運 | 不採用 | 與 x.ospec 並存 | 同專案不保留兩套 spec 系統 |

---

> 本總結濃縮自約 1700 行 Perplexity 對話原文。原文包含重複段落（每則回覆被複製兩次），以及範例檔案模板（已由 x.ospec Toolkit 的 Jinja2 模板取代）。如需查閱原始對話細節，參見原始檔案。
