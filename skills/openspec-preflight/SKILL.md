...
description: PreToolUse hook — 攔截 Write/Edit，確保 Agent 在 OpenSpec 專案中遵守「先有 Change Package，再寫程式碼」的紀律
version: 1.0.0
triggers:
  - openspec preflight
  - openspec guard
...
Agent 要 Write/Edit 檔案
    ...
    ├── 專案沒有 openspec/ 目錄？ → 放行（非 OpenSpec 專案）
    │
    ├── 沒有 active change？
...
```bash
cp openspec-preflight.js ~/.claude/hooks/openspec-preflight.js
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/openspec-preflight.js"
          }
        ]
      }
    ]
...
| Skill | 關係 |
|-------|------|
| `openspec-generator` | 生成的 repo 結構是 preflight 的檢查依據 |
...
A: 只要專案根目錄沒有 `openspec/` 資料夾，hook 就會自動跳過。

**Q: 阻擋時怎麼快速修正？**
A: 執行 `/openspec-generator` skill 建立 Change Package，或手動建立 `openspec/changes/<id>/` 目錄與 `proposal.md`、`design.md`、`tasks.md`。
