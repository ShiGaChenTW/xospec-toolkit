...
| `xospec-preflight.js` | Node.js hook | `~/.claude/hooks/` | PreToolUse 攔截邏輯 |
...
預期：
  ...
  ✅ 生成 .xospec-map.md
  ✅ 生成 xospec/README.md
  ✅ 生成 xospec/specs/auth/spec.md（預設 capability）
  ✅ 生成 xospec/specs/billing/spec.md（預設 capability）
  ✅ 生成 xospec/changes/add-auth/proposal.md
  ✅ 生成 xospec/changes/add-auth/design.md
  ✅ 生成 xospec/changes/add-auth/tasks.md
  ✅ 生成 xospec/changes/add-auth/spec_delta.md（扁平路徑，非 specs/<cap>/spec.md）
  ...
  ✅ git log 有一筆 commit，訊息含 "bootstrap x.ospec"
驗證：
...
預期：
  ✅ xospec/specs/ 下有 payment/, invoice/, notification/ 三個目錄
  ✅ xospec/changes/add-payment-gateway/ 存在
  ✅ proposal.md 含 "Users cannot pay online"
  ✅ proposal.md 含 "E-commerce buyers"
  ✅ .xospec-map.md 含 "payment", "invoice", "notification"
...
#### A3-1: 在既有 xospec repo 新增 change
...
預期：
  ✅ xospec/changes/fix-auth-session-leak/ 存在
  ✅ 含 proposal.md, design.md, tasks.md, spec_delta.md
  ✅ .xospec-map.md 含 "fix-auth-session-leak"
  ✅ xospec/specs/auth/spec.md 未被覆蓋（已存在）
```
...
預期（不提供 --first-change-capability）：
  ...
  ✅ xospec/changes/add-billing-pdf-export/ 存在
```
...
指令：python3 create_repo.py --add-change "add-analytics-dashboard" \
  ...
  --target-dir $TMPDIR/test-add-change
預期：
  ✅ xospec/specs/analytics/spec.md 被新建（之前不存在）
  ✅ xospec/changes/add-analytics-dashboard/ 存在
```
...
```bash
echo '{"tool_name":"Write","tool_input":{"file_path":"<PATH>"}}' | node xospec-preflight.js
echo "Exit: $?"
```

#### B1-1: 非 x.ospec 專案 → 放行
...
#### B1-2: x.ospec 專案、無 active change → 阻擋

```
前置：
  mkdir -p $TMPDIR/test-hook-block/xospec/specs/auth
  # 不建立任何 changes/
...
```
前置：
  mkdir -p $TMPDIR/test-hook-ok/xospec/changes/add-auth
  echo "- [ ] Task 1\n- [x] Task 2\n- [ ] Task 3" > $TMPDIR/test-hook-ok/xospec/changes/add-auth/tasks.md

輸入：{"tool_name":"Write","tool_input":{"file_path":"$TMPDIR/test-hook-ok/src/auth.js"}}
預期：
  ✅ exit code = 0
  ✅ stderr 含 "✅ x.ospec: add-auth [1/3]"
```
...
```
前置：
  mkdir -p $TMPDIR/test-hook-done/xospec/changes/add-auth
  echo "- [x] Task 1\n- [x] Task 2" > $TMPDIR/test-hook-done/xospec/changes/add-auth/tasks.md
...
```
輸入：{"tool_name":"Edit","tool_input":{"file_path":"$TMPDIR/test-hook-ok/xospec/specs/auth/spec.md"}}
預期：
  ✅ exit code = 0
  ✅ 無 stderr（跳過，不檢查）
```
...
```
前置：
  mkdir -p $TMPDIR/test-hook-state/xospec/specs/auth
  ...
  - **Status:** in-progress
  EOF
  mkdir -p $TMPDIR/test-hook-state/xospec/changes/add-auth
  echo "- [ ] Task A" > $TMPDIR/test-hook-state/xospec/changes/add-auth/tasks.md

輸入：{"tool_name":"Write","tool_input":{"file_path":"$TMPDIR/test-hook-state/src/main.js"}}
預期：
  ✅ exit code = 0
  ✅ stderr 含 "✅ x.ospec: add-auth [0/1]"
```
...
#### C2: xospec_map.md.j2 內容驗證

```
result = env.get_template("xospec_map.md.j2").render(**ctx)
預期：
...
#### C3: xospec_map.md.j2 停用 superpowers

```
ctx_no_sp = {**ctx, "enable_superpowers": False}
result = env.get_template("xospec_map.md.j2").render(**ctx_no_sp)
預期：
  ✅ 不含 ".planning/"
```
...
預期：
  ...
  ✅ 含 "xospec/changes/add-billing-export/"
```
...
步驟 2: 驗證 preflight 放行（有 active change + tasks）
  echo '{"tool_name":"Write","tool_input":{"file_path":"'$TMPDIR'/e2e-test/src/auth.js"}}' \
    | node xospec-preflight.js
  → exit code = 0, stderr 含 "✅"

步驟 3: 把所有 tasks 勾完
  sed -i '' 's/- \[ \]/- [x]/g' $TMPDIR/e2e-test/xospec/changes/add-auth/tasks.md
...
步驟 6: 驗證 preflight 再次放行（新 change 有 tasks）
  → exit code = 0, stderr 含 "✅ x.ospec: add-billing-invoice"
```
...
步驟 3: 驗證
  ✅ README.md 仍是 "# My App"
  ✅ xospec/specs/payments/spec.md 存在
  ✅ xospec/specs/users/spec.md 存在
  ✅ git log 有 "brownfield onboard" commit
  ✅ .xospec-map.md 含 "payments" 和 "users"
...
步驟 5: add-change
  python3 create_repo.py --add-change "fix-users-session-bug" \
    --target-dir $TMPDIR/e2e-brown
  ✅ xospec/changes/fix-users-session-bug/ 存在
...
└── fixtures/                 ← 測試用的假專案結構
    ├── plain-project/        ← 無 xospec 的普通專案
    ├── xospec-project/     ← 有完整 xospec 的專案
...
Hook 路徑：~/.claude/hooks/xospec-preflight.js
模板路徑：./templates/
```
